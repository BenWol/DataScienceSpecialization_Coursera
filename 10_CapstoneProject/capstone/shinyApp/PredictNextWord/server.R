...
## set working directory if running locally
...
# definition of the server logic the renders the word predicton algorithm

shinyServer(function(input, output) {
  
  # search for next word with new input
  result <- reactive({
    predictNextword_en(input = gsub('[[:punct:]]', '', input$sentence), ngramData = totalData_slim, useKN = input$model)
  })
  output$result <- renderTable({result()[1:5,]},digits = 3)
  ...
  output$plot <- renderPlot({
    v <- result()
    wordcloud(v$prediction, v$probability,scale=c(5,1.5), colors = brewer.pal(8,"Dark2"), ordered.colors=FALSE)
  })
