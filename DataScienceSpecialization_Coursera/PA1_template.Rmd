...
```{r laoddata, echo = TRUE}
act <- read.csv("activity.csv")
```

and contains `r dim(act)[1]` observations (rows) and includes `r dim(act)[2]` variables (colums). The variables names are `r names(act)` and signify
...
```{r noNA, echo = TRUE, cache=TRUE}
act_noNA <- act[!is.na(act$steps),]
```

The original dataset holds `r sum(is.na(act$steps))` missing values. By looking at the dataset, you can see that always complete days show missing values. To double-check that, I divide the total number of missing values by the number of 5-minute intervals per day (60*24/5=288):
```{r doubleCheckFullDays,echo=TRUE,cache = TRUE}
sum(is.na(act$steps))/(60*24/5)
```
The missing values correspond to `r sum(is.na(act$steps))/(60*24/5)` days.

Next, a histogram of the total number of steps taken PER day (NA values ignored) is created. 
```{r hist_total_number_steps_per_day, echo = TRUE, cache=TRUE}
...
Next, I want to know how many steps throughout the day (in 5min intervals) the participant made, averaged over all `r nrow(tsteps_per_day)` days within october and november of 2012, where his/her steps where detected (without NA values).
...
```{r mean_step_number_per_interval,echo = TRUE, cache = TRUE}
...
```
**The x axis contains `r 60*24/5` points which correspond to all 5-minute intervals of one day!!!**

The 5-minute interval, on average across all the days in the dataset, that contains the maximum number of steps is:
```{r max5minInterval, echo=TRUE, cache=TRUE}
...
```{r totalNumberOfNAs,echo=TRUE,cache = TRUE}
sum(is.na(act$steps))
```
...
```{r fillNAvalues, echo=TRUE,cache=TRUE}
act_fillNA <- act
...
```{r checkNAvalues_inNewDataset, echo=TRUE, cache=TRUE}
sum(is.na(act_fillNA$steps))
```
It should have worked because there are `r sum(is.na(act_fillNA$steps))` NA values.
...
```{r hist_total_number_steps_per_day_fillNA, echo = TRUE, cache=TRUE}
...
```{r weekday_weekend,echo=TRUE,cache=TRUE}
library(dplyr)
...
```
As you can see the ratio between weekend days and weekdays of `r 4608/12960*100`% is about right.
...
```{r mean_step_number_per_interval_weedays,echo = TRUE, cache = TRUE,fig.height=10}
...
```
**Again the x axes contains `r 60*24/5` points which correspond to all 5-minute intervals of one day!!!**
